...
var _XMLHttpRequestConstants = require("./XMLHttpRequestConstants");

var _XMLHttpRequestStates = require("./XMLHttpRequestStates");
...
class XMLHttpRequest {
  // States
  static get UNSENT() {
    return _XMLHttpRequestStates.UNSENT;
  }

  static get OPENED() {
    return _XMLHttpRequestStates.OPENED;
  }

  static get HEADERS_RECEIVED() {
    return _XMLHttpRequestStates.HEADERS_RECEIVED;
  }

  static get LOADING() {
    return _XMLHttpRequestStates.LOADING;
  }

  static get DONE() {
    return _XMLHttpRequestStates.DONE;
  }
  ...
  constructor(options) {
    ...
    this._response = null; // Disable header blacklist.
    // Not part of XHR specs.

    this._disableHeaderCheck = false; // Send flag
    ...
    this.readyState = _XMLHttpRequestStates.UNSENT; // default ready state change handler in case one is not set or is set late
  ...
  _isAllowedHttpHeader(header) {
    return this._disableHeaderCheck || header && _XMLHttpRequestConstants.FORBIDDEN_REQUEST_HEADERS.indexOf(header.toLowerCase()) === -1;
  }
  /**
  ...
  _isAllowedHttpMethod(method) {
    return method && _XMLHttpRequestConstants.FORBIDDEN_REQUEST_METHODS.indexOf(method) === -1;
  }
  /**
  ...
  open(method, url, async, user, password) {
    this.abort();
    this._errorFlag = false;
    this._abortedFlag = false; // Check for valid request method
    ...
    this.setState(_XMLHttpRequestStates.OPENED);
  }
  /**
  ...
  setRequestHeader(header, value) {
    if (this.readyState != _XMLHttpRequestStates.OPENED) {
  ...
  getResponseHeader(header) {
    if (typeof header === 'string' && this.readyState > _XMLHttpRequestStates.OPENED && this._response.headers[header.toLowerCase()] && !this._errorFlag) {
  ...
  getAllResponseHeaders() {
    if (this.readyState < _XMLHttpRequestStates.HEADERS_RECEIVED || this._errorFlag) {
      return '';
    }
  ...
  send(data) {
    ...
    let url = _url.default.parse(this._requestSettings.url);

    let host; // Determine the server
    ...
    if (local) {
      ...
      if (this._requestSettings.async) {
        _fs.default.readFile(url.pathname, 'utf8', (error, data) => {
          if (error) {
            this.handleError(error);
          } else {
            this.status = 200;
            this.responseText = data;
            this.setState(_XMLHttpRequestStates.DONE);
          }
        });
      } else {
        try {
          this.responseText = _fs.default.readFileSync(url.pathname, 'utf8');
          this.status = 200;
          this.setState(_XMLHttpRequestStates.DONE);
        } catch (e) {
          this.handleError(e);
    ...
    let port = url.port || (ssl ? 443 : 80); // Add query string if one is used

    let uri = url.pathname + (url.search ? url.search : ''); // Set the Host header or the server may reject the request
    ...
    const errorHandler = error => this.handleError(error); // Reset error flag


    this._errorFlag = false; // Handle async requests

    if (this._requestSettings.async) {
      // Use the proper protocol
      let doRequest = ssl ? _https.default.request : _http.default.request; // Request is being sent, set send flag

      this._sendFlag = true; // As per spec, this is called here for historical reasons.
      ...
      let responseHandler = resp => {
        ...
        if (this._response.statusCode === 302 || this._response.statusCode === 303 || this._response.statusCode === 307) {
          ...
          let url = _url.default.parse(this._requestSettings.url); // Set host var in case it's used later
        ...
        this.setState(_XMLHttpRequestStates.HEADERS_RECEIVED);
        ...
        this._response.on('data', chunk => {
          ...
          if (this._sendFlag) {
            this.setState(_XMLHttpRequestStates.LOADING);
          }
        });
        ...
        this._response.on('end', () => {
          if (this._sendFlag) {
            ...
            this._sendFlag = false; // Discard the 'end' event if the connection has been aborted
      ...
      // Synchronous
      const syncProc = (0, _child_process.spawnSync)(process.argv[0], [`${__dirname}/XMLHttpRequestSync.mjs`, ssl, JSON.stringify(options), data]);
  ...
  abort() {
    ...
    if (this.readyState !== _XMLHttpRequestStates.UNSENT && (this.readyState !== _XMLHttpRequestStates.OPENED || this._sendFlag) && this.readyState !== _XMLHttpRequestStates.DONE) {
      this._sendFlag = false;
      this.setState(_XMLHttpRequestStates.DONE);
    }

    this.readyState = _XMLHttpRequestStates.UNSENT;
  }
  /**
  ...
  dispatchEvent(event) {
    if (typeof this['on' + event] === 'function') {
      if (this.readyState === _XMLHttpRequestStates.DONE) {
  ...
  setState(state) {
    if (this.readyState === state || this.readyState === _XMLHttpRequestStates.UNSENT && this._abortedFlag) {
      return;
    }

    this.readyState = state;

    if (this._requestSettings.async || this.readyState < _XMLHttpRequestStates.OPENED || this.readyState === _XMLHttpRequestStates.DONE) {
      this.dispatchEvent('readystatechange');
    }
    ...
    if (this.readyState === _XMLHttpRequestStates.DONE) {
      ...
      this.dispatchEvent(fire); // @TODO figure out InspectorInstrumentation::didLoadXHR(cookie)
